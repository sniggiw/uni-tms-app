        ...
        <view class="content">
            ...
            <view v-show="current === 2">
                <view class="search-btn">
                    <uni-search-bar
                        v-model="orderCode"
                        @confirm="completionOrderProcess"
                        :radius="100"
                        cancelButton="none"
                        placeholder="请输入订单编码"
                    ></uni-search-bar>
...
<script setup>
import { orderPendingIndex, orderPendingList, orderProcessing } from "@/api/common";
...
const orderPendingIndexData = () => {
    ...
    orderPendingIndex().then((res) => {
        uni.hideLoading();
        if (res.code === 200) {
            orderNum.value = res.data.reduce((prev, item) => {
                return prev + item.orderCount;
            }, 0); //待我处理-Badge
